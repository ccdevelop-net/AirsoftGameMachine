...
//-----------------------------------------------------------------------------
static void dump(const JsonArray_t &values, string &out) {
  bool first = true;
  out += "[";
...
//-----------------------------------------------------------------------------
static void dump(const JsonObject_t &values, string &out) {
  bool first = true;
  out += "{";
...
protected:
  ...
  const T m_value;
  void dump(string &out) const override { Airsoft::Classes::dump(m_value, out); }
};
...
class JsonArray final : public Value<Json::Type::ARRAY, JsonArray_t> {
  const JsonArray_t &array_items() const override { return m_value; }
  const Json & operator[](size_t i) const override;
public:
  explicit JsonArray(const JsonArray_t &value) : Value(value) {}
  explicit JsonArray(JsonArray_t &&value)      : Value(move(value)) {}
};

class JsonObject final : public Value<Json::Type::OBJECT, JsonObject_t> {
  const JsonObject_t &object_items() const override { return m_value; }
  const Json & operator[](const string &key) const override;
public:
  explicit JsonObject(const JsonObject_t &value) : Value(value) {}
  explicit JsonObject(JsonObject_t &&value)      : Value(move(value)) {}
};
...
Json::Json(const JsonArray_t &values)  : m_ptr(make_shared<JsonArray>(values)) {}
Json::Json(JsonArray_t &&values)       : m_ptr(make_shared<JsonArray>(move(values))) {}
Json::Json(const JsonObject_t &values) : m_ptr(make_shared<JsonObject>(values)) {}
Json::Json(JsonObject_t &&values)      : m_ptr(make_shared<JsonObject>(move(values))) {}
